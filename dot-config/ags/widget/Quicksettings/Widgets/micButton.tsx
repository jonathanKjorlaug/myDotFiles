...
export default function MicQS() {
    ...
    return (
        <QSButton
            cssClasses={bind(wireplumber?.default_microphone!, "mute").as(
                (mute) => {
            ...
            iconName={bind(wireplumber?.default_microphone!, "mute").as(
                (mute) =>
                    mute
            ...
            label={"Mic Access"}
            status={bind(wireplumber?.get_default_microphone()!, "mute").as(
                (m) => (m ? "Off" : "On"),
            )}
            onClicked={() => {
